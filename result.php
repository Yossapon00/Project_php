<?php
    ...
    $check3 = mysqli_query($conn,"SELECT * FROM Check3 order by Class ");
    $data = mysqli_fetch_array($check1);
    $data2 = mysqli_fetch_array($check2);
    $data3 = mysqli_fetch_array($check3);
    //echo $data[3];
    //echo $data2[3];
    if($id === $data[0]){
        //echo $data[3];
    ...
    if($id === $data3[0]){
        //echo $data2[3];
        $sql = mysqli_query($conn,"SELECT * FROM subject03");
    }
...
<body>
    ...
    <script>
        document.querySelector('.Pay').onclick = function(){
            alert("คุณได้ทำการขอใบชำระเงินเสร็จเรียบร้อยแล้ว \n\nเจ้าหน้าที่จะทำการตรวจสอบและจะส่งใบชำระเงินให้ แก่ท่านผ่านทางอีเมลส่วนตัวของทางโรงเรียน");
        }
    </script>
